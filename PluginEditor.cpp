    ...
    : AudioProcessorEditor(&p), proc(p), curve(p.apvts)
{
    ...
    // IMPORTANT : sans ceci, l'hote refuse d'agrandir la fenetre
    // et le mode ADVANCED reste invisible (hors ecran).
    setResizable(true,false);
    setResizeLimits(660,470,660,700);
    setSize(660,700);   // taille FIXE : aucun DAW n'a besoin de redimensionner
    setAdvanced(false); // etat initial : SIMPLE (courbe visible, trims caches)
}
...
void RoomCorrectorAudioProcessorEditor::setAdvanced (bool on)
{
    ...
    // La courbe reste visible dans les 2 modes (retour visuel permanent).
    // Seuls les reglages fins apparaissent en ADVANCED.
    curve.setVisible(true);
    for (auto* c : { &bodyS,&presS,&airS,&sibFS })  c->setVisible(on);
    for (auto* l : { &bodyL,&presL,&airL,&sibFL })  l->setVisible(on);

    resized();   // repositionne
    repaint();   // redessine les libelles
}
...
void RoomCorrectorAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.drawText("COURBE DE CORRECTION · TEMPS REEL",28,426,280,12,juce::Justification::left);
    if (advanced)
        g.drawText("REGLAGES FINS",28,556,200,12,juce::Justification::left);
    else
        g.drawText("ADVANCED  >  reglages fins au millimetre",28,556,320,12,juce::Justification::left);
...
void RoomCorrectorAudioProcessorEditor::resized()
{
    ...
    // Zone basse : toujours positionnee (la visibilite fait l'affichage)
    curve.setBounds(24,440,W-48,108);
    const int tw=(W-48)/4;
    const int ty=570;
